...
class Util
{
  ...
  function LoadJPEG ($img_name = null) 
  {
    if (ImageTypes() & IMG_JPEG)
      {
	$im = @ImageCreateFromJPEG($img_name) or die ("Cannot create JPEG stream");
	//	$im = 0;
	if ($im) 
	  {
	    return $im;
  ...
  function LoadPNG ($img_name = null) 
  {
    ...
    elseif(is_null($img_name))
      {
	$error_string = "Filename null";
      }
    else
      {
  ...
  function writeJPEG(& $image_resource, $file_name = null)
  {
    if(!is_null($file_name))
      {
	if (ImageTypes() & IMG_JPEG)
	  {
	    //	    echo "writeJPEG(): Writing file $file_name<br/>";
	    imageinterlace($image_resource);	
	    imageJPEG($image_resource, $file_name);
	    return TRUE;
	  }
	else return FALSE;
      }
  ...
  function writePNG(& $image_resource, $file_name = null)
  {
    if(!is_null($file_name))
      {
	if (ImageTypes() & IMG_PNG)
	  {
	    //	    echo "writeJPEG(): Writing file $file_name<br/>";
	    imageinterlace($image_resource);	
	    imagePNG($image_resource, $file_name);
	    return TRUE;
	  }
	else return FALSE;
      }
  ...
  function writeGIF(& $image_resource, $file_name = null)
  {
    if(!is_null($file_name))
      {
	if (ImageTypes() & IMG_GIF)
	  {
	    //	    echo "writeJPEG(): Writing file $file_name<br/>";
	    imageinterlace($image_resource);	
	    imageGIF($image_resource, $file_name);
	    return TRUE;
	  }
	else return FALSE;
      }
